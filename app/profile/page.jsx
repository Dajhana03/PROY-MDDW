...
const IconLeaf = (props) => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" {...props}>
    <path
      d="M5 21c0-8 5-15 14-15 0 9-5 15-14 15z"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinejoin="round"
    />
    <path
      d="M5 21c3-4 6-7 12-11"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinecap="round"
    />
...
const IconFlame = (props) => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" {...props}>
    <path
      d="M12 2c1 4-4 5-4 10a4 4 0 0 0 8 0c0-2-1-3-1-3s2 1 2 4a6 6 0 1 1-12 0C5 8 9 8 12 2z"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinejoin="round"
    />
...
const IconUsers = (props) => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" {...props}>
    ...
    />
    <path
      d="M15.5 5.5c1.3.4 2.2 1.5 2.2 2.9 0 1.4-.9 2.5-2.2 2.9"
      stroke="currentColor"
      strokeWidth="1.6"
      strokeLinecap="round"
    />
    <path
      d="M17.5 14.7c2 .6 3.5 2.3 3.5 4.3"
      stroke="currentColor"
      strokeWidth="1.6"
      strokeLinecap="round"
    />
...
const IconRecycle = (props) => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" {...props}>
    <path
      d="M7 19H4.8a2 2 0 0 1-1.7-3l1.6-2.7"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinecap="round"
      strokeLinejoin="round"
    ...
    />
    <path
      d="M13.5 3.6h2.6a2 2 0 0 1 1.7 1l1.5 2.7"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M20.8 12.6l1.4 2.4-1.4 2.4h-4.9"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinecap="round"
      strokeLinejoin="round"
    ...
    />
    <path
      d="M9.2 19.9H6.6a2 2 0 0 1-1.7-1l-1.2-2.2"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinecap="round"
      strokeLinejoin="round"
...
const IconGift = (props) => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" {...props}>
    ...
    <path
      d="M12 8c-1.5 0-4-1-4-3s1.5-2.5 2.7-1.5S12 6 12 8z"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinejoin="round"
    />
    <path
      d="M12 8c1.5 0 4-1 4-3s-1.5-2.5-2.7-1.5S12 6 12 8z"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinejoin="round"
    />
...
const IconInbox = (props) => (
  <svg width="34" height="34" viewBox="0 0 24 24" fill="none" {...props}>
    ...
    />
    <path
      d="M5 5h14l2 7v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-6l2-7z"
      stroke="currentColor"
      strokeWidth="1.7"
      strokeLinejoin="round"
    />
...
export default function ProfilePage() {
  ...
  // Solicitudes que otros usuarios hicieron sobre MIS donaciones
    ...
    const load = async () => {
      try {
        ...
        const dSnap = await getDocs(
          query(collection(db, "donations"), where("ownerId", "==", uid)),
        );
  ...
  // Escucha en tiempo real las solicitudes que recibí en mis donaciones
  useEffect(() => {
    if (!uid) return;
    ...
    const unsub = onSnapshot(
      ...
      (err) => console.error("Error al escuchar solicitudes recibidas:", err),
    );
  ...
  const handleRequestDecision = async (requestId, decision) => {
    try {
      ...
      showToast(
        decision === "aceptado" ? "Solicitud aceptada" : "Solicitud rechazada",
      );
    } catch (e) {
  ...
  const role = ROLE_MAP[userData.user_type] || {
    label: "Usuario",
    color: "#64748b",
  };
  const displayName =
    `${userData.first_name || "Usuario"} ${userData.last_name || ""}`.trim();
  const displayEmail =
    userData.email || userData.authEmail || "Correo no registrado";
  const initials =
    `${userData.first_name?.[0] || "U"}${userData.last_name?.[0] || ""}`.toUpperCase();

  // Puntos y Nivel
  const totalPoints =
    (userData.points || 0) +
    donations.reduce((sum, d) => sum + (Number(d.puntos) || 50), 0);
  ...
  const isAnswered = (m) =>
    m.status === "respondido" || !!(m.reply && m.reply.trim());
        ...
        <div className={styles.heroMetaRow}>
          <div className={styles.growthStem}>
            <div className={styles.stemTrack}>
              <div
                className={styles.stemFill}
                style={{ width: `${stemPercent}%` }}
              />
              {LEVELS.map((level) => {
            ...
            <div className={styles.stemLabels}>
              {LEVELS.map((level, i) => {
                ...
                const translateX =
                  i === 0 ? "0%" : i === LEVELS.length - 1 ? "-100%" : "-50%";
                return (
                  <span
                    key={level.name}
          ...
          <div className={styles.profileView}>
            {profileCompletion < 100 && (
              <div className={styles.progressSection}>
                <div className={styles.progressHeader}>
                  <span>
                    Completa tu perfil para ganar más confianza (
                    {profileCompletion}%)
                  </span>
                  <button
            ...
            <div className={styles.infoGrid}>
              <div className={styles.infoItem}>
                ...
                <p className={styles.infoValue}>
                  {userData.campus || "No especificada"}
                </p>
              </div>
              <div className={styles.infoItem}>
                <span className={styles.infoLabel}>Facultad / Carrera</span>
                <p className={styles.infoValue}>
                  {userData.faculty || "No especificada"}
                </p>
              </div>
              <div className={styles.infoItem} style={{ gridColumn: "1 / -1" }}>
                <span className={styles.infoLabel}>Sobre mí</span>
                <p className={styles.infoValue}>
                  {userData.bio || "¡Hola! Soy parte de la comunidad EcoCanje."}
                </p>
              </div>
            </div>
            ...
            <div className={styles.badgesGrid}>
              <div
                className={`${styles.badgeCard} ${donations.length >= 1 ? styles.badgeActive : ""}`}
              >
              ...
              <div
                className={`${styles.badgeCard} ${totalPoints >= 150 ? styles.badgeActive : ""}`}
              >
                <span className={styles.badgeIconWrap}>
                  <IconFlame />
                </span>
              ...
              <div
                className={`${styles.badgeCard} ${userData.bio && userData.campus ? styles.badgeActive : ""}`}
              >
                <span className={styles.badgeIconWrap}>
                  <IconUsers />
                </span>
          ...
          <div className={styles.section}>
            ...
            {donations.length === 0 ? (
              ...
              <div className={styles.activityList}>
                {donations.map((d) => {
                  const donationRequests = incomingRequests.filter(
                    (r) => r.donationId === d.id,
                  );
                  return (
                    <div key={d.id} className={styles.donationBlock}>
                      <div className={styles.activityCard}>
                        ...
                        <div>
                          ...
                          <p className={styles.activitySub}>
                            {d.type || "Artículo"} · {d.location || "Ubicación"}
                          </p>
                        </div>
                        <span
                          className={`${styles.activityStatus} ${styles[d.status] || ""}`}
                        >
                          {d.status || "Activa"}
                        </span>
                      </div>
                            ...
                            <div key={r.id} className={styles.requestCard}>
                              <div className={styles.requestHeader}>
                                <span className={styles.requestUser}>
                                  <IconUsers />{" "}
                                  {r.userName || r.userEmail || "Usuario"}
                                </span>
                                <span
                              ...
                              {r.mensaje && (
                                <p className={styles.requestMsg}>{r.mensaje}</p>
                              )}
                              ...
                              {(!r.status || r.status === "pendiente") && (
                                <div className={styles.requestActions}>
                                  <button
                                    ...
                                    onClick={() =>
                                      handleRequestDecision(r.id, "aceptado")
                                    }
                                  >
                                    <IconCheck /> Aceptar
                                  </button>
                                  <button
                                    ...
                                    onClick={() =>
                                      handleRequestDecision(r.id, "rechazado")
                                    }
                                  >
                                    <IconX /> Rechazar
                                  </button>
          ...
          <div className={styles.section}>
            ...
            {supportLoading ? (
              ...
              <div className={styles.emptySupport}>
                ...
                <a href="/contacto" className={styles.linkBtn}>
                  Ir a Contacto
                </a>
              </div>
            ) : (
